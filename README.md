<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/version-0.9.31-287464?style=flat-square" alt="Version 0.9.31" />
...
| 🏷️ 有序归类 | 知识库与标签隔离；筛选结果全选、整组管理、批量标签 / 移动 / 收藏、删除与撤销 |
| 💬 微信收件 | 绑定个人微信收集入口，文字与图片按天追加为图文笔记，也可手动分篇；无需同一局域网，需账号支持 ClawBot |
...
| 使用场景 | 连接方式 |
| :--- | :--- |
| 在服务器本机查看 | 打开 `http://localhost:3741` |
| 手机 / 平板 / 其他电脑查看 | 连接服务器所在局域网，打开 `http://服务器局域网IP:3741` |
| 在外通过微信投递 | 手机与 ZNote 主机分别联网即可，见 [微信收件箱](#微信收件箱) |
| 在外直接查看知识库网页 | 另行配置远程访问；项目不会自动为知识库生成公网地址 |

手机上的 `localhost` 指手机自身；访问电脑上的 ZNote 时请使用电脑的局域网 IP。所有设备连接同一个服务，读取同一份知识库；服务停止后无法访问网页或继续收件。网络与部署配置见 [部署指南](docs/DEPLOYMENT.md)。
...
详细操作、备份说明与协议依据见 **[微信接入说明](integrations/weixin/README.md)**。
...
| [微信接入](integrations/weixin/README.md) | 扫码绑定、联网条件、收件范围、故障与迁移 |
| [普通扩展](extensions/clipper/README.md) | 悬停图片、视频嗅探、正文采集与连接设置 |
| [Pixiv 增强版](integrations/pixiv/README.md) | 作品与目录入库、作者标签、收藏联动、动图与小说 |
