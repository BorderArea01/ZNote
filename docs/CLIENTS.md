...
### 手机分享与悬浮采集（Android 0.10.0-beta.6）

在其他 App 或浏览器选择「分享 → 保存到 ZNote」，也可打开 ZNote 顶栏「采集」粘贴分享文字。首次先连接并登录知识库；之后分享页沿用登录状态，记住上次选择的目标知识库。

**悬浮入口**：在「采集 → 悬浮采集设置」选择「开启 / 管理采集辅助」，按系统提示开启「ZNote 点击采集」。小窗可拖动，松手贴到左右边缘，并记住位置。点开后选择「获取当前页面」：受支持浏览器读取完整地址栏，小红书 / 抖音 / B 站尝试点开分享、复制链接，再带到原来的采集页确认目标知识库。也可手动打开作品分享菜单后再点采集。遇到不支持的布局，可用「粘贴链接」或系统分享继续。浮窗中的「收起」「关闭」均可直接触屏操作。悬浮采集使用独立进程，知识库网页繁忙时仍可展开和收起；识别期间可取消，超时会显示原因并允许重试。B 站等 App 的视频列表暂不支持整页批量采集，请先打开具体视频；不会从多个分享入口中随意选一个。
