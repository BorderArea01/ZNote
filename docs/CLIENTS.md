...
ZNote 的图片、笔记和数据库保存在运行服务的电脑或 NAS 上。客户端连接同一地址，看到同一份内容；手机无需重复导入素材。Android 客户端当前为 **0.10.0-beta.11**，桌面客户端为 **0.10.0-beta.1**，均属于公开测试版。
...
**悬浮入口**：在「采集 → 悬浮采集设置」选择「开启 / 管理采集辅助」，按系统提示开启「ZNote 点击采集」。小窗可拖动，松手贴到左右边缘，并记住位置；收起时隐藏 Z 图标，仅保留窄把手。点开后选择「获取当前页面」：受支持浏览器读取完整地址栏，小红书 / 抖音 / B 站尝试点开分享、复制链接，再在原 App 上方的小型采集面板读取、确认目标知识库并保存，不打开整页知识库。面板可拖动；读取失败可直接在面板输入框长按粘贴，也可点「读取剪贴板 / 粘贴链接」，无需切到 ZNote 主界面。也可手动打开作品分享菜单后再点采集。遇到不支持的布局，可用「粘贴链接」或系统分享继续。浮窗中的「收起」「关闭」均可直接触屏操作。悬浮采集使用独立进程，知识库网页繁忙时仍可展开和收起；识别期间可取消，超时会显示原因并允许重试。B 站等 App 的视频列表暂不支持整页批量采集，请先打开具体视频；不会从多个分享入口中随意选一个。
...
小红书 `xhslink.cn` 分享链接支持自动填入；新版分享页的图集、作者及视频播放地址由服务器解析。B 站手机页面链接会转换为下载器支持的作品地址。抖音普通页面缺少数据时，服务器会按需启动独立的匿名 Edge / Chrome 会话，只读取当前作品的数据，关闭图片与视频自动加载，最多等待 30 秒后关闭浏览器。服务器需安装 Edge / Chrome，或用 `ZNOTE_CAPTURE_BROWSER` 指定 Chromium 可执行文件；不会读取日常浏览器的账号或配置。平台未提供完整作品或要求登录时仍可能失败，失败任务可重试。
