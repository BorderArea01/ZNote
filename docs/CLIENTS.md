...
采集辅助是可选的 Android 无障碍服务，**只在点击时读取受支持 App 的页面控件**；平时不扫描、不录屏，不读取聊天内容。首次开启前会展示读取范围与用途。关闭悬浮窗后不再采集，也可随时在系统设置停用服务。它不绕过平台登录，不能直接读取其他 App 内部的媒体文件。系统限制服务、无障碍控件不可见或 App 改版时，使用系统分享入口。
