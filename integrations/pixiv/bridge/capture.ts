...
root.innerHTML = `<style>:host{position:fixed;bottom:20px;left:20px;z-index:2147483646;font:14px system-ui;color:#ecedf6}.box{background:#1c202b;border:1px solid #a8b5d3;border-radius:14px;padding:10px;box-shadow:0 5px 25px #0004;display:flex;gap:8px;align-items:center;max-width:calc(100vw - 60px);flex-wrap:wrap}button{font:inherit;color:inherit;background:#2a3040;border:1px solid #a8b5d3;border-radius:9px;padding:8px 12px;cursor:pointer}button:disabled{opacity:.5}#save{background:#5865ce;border-color:#808def}button:focus-visible{outline:2px solid #bec7ff;outline-offset:3px}button:hover:not(:disabled){filter:brightness(1.2)}small{max-width:300px;overflow-wrap:anywhere}</style><div class="box"><button id="save">保存抓取结果到 ZNote</button><button id="jobs" title="连接设置与入库历史">任务 / 设置</button><small id="status"></small></div>`
