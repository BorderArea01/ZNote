...
const announce=()=>{if(group)parent.postMessage({type:'znote-inline-progress',id,title:group.title,height:Math.ceil(document.body.getBoundingClientRect().height),running:!!library?.running,done:(group.saveStates||[]).filter(s=>['done','duplicate'].includes(s)).length,total:group.images.length},group.owner.origin);};
