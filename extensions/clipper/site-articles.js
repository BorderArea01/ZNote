...
export async function siteArticle(document, location, fetcher = fetch) {
  ...
  if(host==='pixiv.net') {
    ...
    for(const [index,page] of pages.entries()) {
      ...
      const img=clone.createElement('img');img.src=url.href;img.alt=`第 ${index+1} 张`;article.append(img);
    }
  ...
  if(['pawchive.pw','pawchive.st'].includes(host)) {
    ...
    for(const link of article.querySelectorAll('figure a[href],a.fileThumb[href]')){
      ...
      const img=clone.createElement('img');img.src=url;img.alt=link.querySelector('img')?.alt||`作品配图 ${seen.size}`;link.replaceWith(img);
    }
