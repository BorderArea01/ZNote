...
globalThis.ZNotePawControls=class {
  constructor(root,gallery,send){
    ...
    this.buttons=[];const action=(parent,text,label,mode)=>{const b=make('button',text);b.type='button';b.setAttribute('aria-label',label);b.onclick=e=>{if(!e.isTrusted)return;e.preventDefault();e.stopPropagation();this.run(mode);};this.buttons.push(b);parent.append(b);};
    action(this.toolbar,'↓ 本图','下载当前图片','single');action(this.toolbar,'↓ 套图','下载此作品套图','download');action(this.toolbar,'＋ 入库','此作品整组入库','save');
    action(actions,'下载套图','下载当前套图','download');action(actions,'整组入库','保存当前套图到知识库','save');
  ...
  async run(mode){
    ...
    try{
      ...
      if(mode==='single'||group.images.length===1){const index=mode==='single'?group.start_index:0;const resource=await this.send({type:'hover-resource',url:group.images[index],title:group.title+' · '+(index+1),source_url:group.source_url});const result=await this.send({type:'media-action',id:resource.id,action:mode==='save'?'save':'download'});this.status.textContent=result.message;this.gallery.show();}
