export async function openGallery(group, sender) {
  const page = new URL(sender.url), source = new URL(group?.source_url);
  if (!/^https?:$/.test(page.protocol) || source.href !== page.href || !Array.isArray(group.images) || group.images.length < 2 || group.images.length > 200) throw Error('作品图片组无效，请刷新网页后重试');
  ...
  await chrome.storage.session.set({['gallery-'+id]:{images,title:String(group.title||'作品图片').slice(0,180),source_url:source.href,created:Date.now()}});
