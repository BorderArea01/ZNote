...
public class SmokeRunner extends Instrumentation {
    ...
    private void overlayTests()throws Exception{
        ...
        overlayTouch("Z");ActivityMonitor pasteMonitor=new ActivityMonitor(FloatingShareActivity.class.getName(),null,false);addMonitor(pasteMonitor);overlayTouch("粘贴链接");FloatingShareActivity fallback=(FloatingShareActivity)waitForMonitorWithTimeout(pasteMonitor,10000);removeMonitor(pasteMonitor);if(fallback==null)throw new Exception("Floating paste action did not open its Activity");nativeUntil(fallback,"链接还没带进来");nativeUntil(fallback,"未读到本次分享链接");
        touchText("去浏览");until("document.body.innerText.includes('我的知识库')");shell("input keyevent 4");Thread.sleep(400);nativeUntil(fallback,"未读到本次分享链接");checkpoint("Floating save panel can open the library and return without discarding the pending share");
