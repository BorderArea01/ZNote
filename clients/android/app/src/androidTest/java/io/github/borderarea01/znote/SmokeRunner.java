...
public class SmokeRunner extends Instrumentation {
    ...
    private boolean interactiveWindows;
    private UiAutomation automation(){UiAutomation ui=getUiAutomation(UiAutomation.FLAG_DONT_SUPPRESS_ACCESSIBILITY_SERVICES);if(!interactiveWindows){android.accessibilityservice.AccessibilityServiceInfo info=ui.getServiceInfo();if(info!=null){info.flags|=android.accessibilityservice.AccessibilityServiceInfo.FLAG_RETRIEVE_INTERACTIVE_WINDOWS;ui.setServiceInfo(info);}interactiveWindows=true;}return ui;}
    ...
    private String accessibleTree(android.view.accessibility.AccessibilityNodeInfo node){if(node==null)return "";StringBuilder value=new StringBuilder();CharSequence text=node.getText(),description=node.getContentDescription();if(text!=null&&!text.toString().isBlank())value.append(text).append(" | ");if(description!=null&&!description.toString().isBlank())value.append(description).append(" | ");for(int i=0;i<node.getChildCount();i++){android.view.accessibility.AccessibilityNodeInfo child=node.getChild(i);if(child!=null){value.append(accessibleTree(child));child.recycle();}}return value.toString();}
    private String accessibleWindows(){StringBuilder value=new StringBuilder();for(android.view.accessibility.AccessibilityWindowInfo window:automation().getWindows()){android.view.accessibility.AccessibilityNodeInfo root=window.getRoot();if(root==null)continue;value.append("[type=").append(window.getType()).append(",active=").append(window.isActive()).append(",focused=").append(window.isFocused()).append(",pkg=").append(root.getPackageName()).append("] ").append(accessibleTree(root)).append("\n");root.recycle();}return value.toString();}
    private void nativeUntil(Activity target,String expected)throws Exception{long deadline=System.currentTimeMillis()+30000;while(System.currentTimeMillis()<deadline){String[] value={""};if(target==null){for(android.view.accessibility.AccessibilityWindowInfo w:automation().getWindows())if(w.getType()==android.view.accessibility.AccessibilityWindowInfo.TYPE_APPLICATION){android.view.accessibility.AccessibilityNodeInfo r=w.getRoot();if(r==null)continue;if(getTargetContext().getPackageName().contentEquals(r.getPackageName())){java.util.List<android.view.accessibility.AccessibilityNodeInfo> matches=r.findAccessibilityNodeInfosByText(expected);if(!matches.isEmpty())value[0]=expected;for(android.view.accessibility.AccessibilityNodeInfo n:matches)n.recycle();}r.recycle();if(!value[0].isEmpty())break;}}else runOnMainSync(()->value[0]=nativeText(target.getWindow().getDecorView()));if(value[0].contains(expected))return;Thread.sleep(250);}throw new Exception("Native share did not reach: "+expected+"; windows="+accessibleWindows());}
    ...
    private void overlayTests()throws Exception{
        ...
        runOnMainSync(()->((android.content.ClipboardManager)getTargetContext().getSystemService(Context.CLIPBOARD_SERVICE)).setPrimaryClip(android.content.ClipData.newPlainText("fixture","")));
        overlayTouch("Z");overlayTouch("粘贴链接");nativeUntil(null,"链接还没带进来");nativeUntil(null,"未读到本次分享链接");
        touchText("去浏览");until("document.body.innerText.includes('我的知识库')");shell("input keyevent 4");Thread.sleep(400);nativeUntil(null,"未读到本次分享链接");checkpoint("Floating save panel can open the library and return without discarding the pending share");
        ...
        touchText("读取剪贴板");nativeUntil(null,"http://127.0.0.1/manual-fallback");
        ...
        overlayControl("采集当前作品");overlayTouch("粘贴链接");nativeUntil(null,"保存到 ZNote");boolean independent=released.getCount()>0;released.await();if(!independent)throw new Exception("Floating panel waited for the blocked library process");checkpoint("Floating handle and native capture Activity respond above another app while library thread is blocked: "+panelLatency(600)+"ms");
