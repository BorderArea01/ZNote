...
public class CaptureAssistService extends AccessibilityService {
    static volatile CaptureAssistService current;
    ...
    private void dock(){
        ...
        layout.y=Math.max(dp(24),Math.min(height()-dp(expanded?244:96),anchor));manager.updateViewLayout(bubble,layout);
    }
    private void render(){
    ...
    private void drag(View handle){handle.setOnTouchListener(new View.OnTouchListener(){float x,y;int ox,oy;boolean moved;final int slop=ViewConfiguration.get(CaptureAssistService.this).getScaledTouchSlop();public boolean onTouch(View v,MotionEvent e){switch(e.getActionMasked()){
        ...
        case MotionEvent.ACTION_MOVE:float dx=e.getRawX()-x,dy=e.getRawY()-y;if(Math.hypot(dx,dy)>slop)moved=true;if(moved){v.setPressed(false);layout.x=Math.max(0,Math.min(width()-layout.width,ox+(int)dx));layout.y=Math.max(dp(24),Math.min(height()-bubble.getHeight()-dp(32),oy+(int)dy));manager.updateViewLayout(bubble,layout);}return true;
    ...
    private void message(String text){busy=false;expanded=true;render();if(status!=null){status.setText(text);status.setVisibility(View.VISIBLE);}}
    ...
    private AccessibilityNodeInfo page(String owner){
        ...
        for(AccessibilityWindowInfo w:getWindows())if(w.getType()==AccessibilityWindowInfo.TYPE_APPLICATION&&(owner!=null||w.isActive()||w.isFocused())){AccessibilityNodeInfo root=w.getRoot();if(root!=null){if(owner==null||owner.equals(pkg(root)))return root;root.recycle();}}
        AccessibilityNodeInfo root=getRootInActiveWindow();if(root!=null&&owner!=null&&!owner.equals(pkg(root))){root.recycle();return null;}return root;
    }
    ...
    private AccessibilityNodeInfo find(AccessibilityNodeInfo root,Match match,int token)throws InterruptedException{
        if(root==null)return null;ArrayDeque<AccessibilityNodeInfo> queue=new ArrayDeque<>();queue.add(root);long end=SystemClock.uptimeMillis()+1800;int count=0;
        try{while(!queue.isEmpty()&&count++<1800&&SystemClock.uptimeMillis()<end){check(token);AccessibilityNodeInfo n=queue.removeFirst();if(Build.VERSION.SDK_INT<33&&!n.refresh()){n.recycle();continue;}if(n.isVisibleToUser()&&!n.isPassword()&&match.test(n))return n;for(int i=0;i<n.getChildCount()&&queue.size()<1800;i++){AccessibilityNodeInfo child=n.getChild(i);if(child!=null)queue.add(child);}n.recycle();}return null;}finally{for(AccessibilityNodeInfo n:queue)n.recycle();}
    }
    ...
    private AccessibilityNodeInfo button(String owner,boolean copy,int token)throws InterruptedException{
        if(Build.VERSION.SDK_INT>=33)clearCache();
        for(AccessibilityWindowInfo w:getWindows())if(w.getType()==AccessibilityWindowInfo.TYPE_APPLICATION){AccessibilityNodeInfo root=w.getRoot();if(root==null)continue;if(!owner.equals(pkg(root))){root.recycle();continue;}
            // Provider-side text search reaches native dialogs and large lists without walking each row.
            for(String query:copy?new String[]{"复制链接","复制分享链接","Copy link"}:new String[]{"分享","转发","Share"}){check(token);java.util.List<AccessibilityNodeInfo> matches=root.findAccessibilityNodeInfosByText(query);AccessibilityNodeInfo hit=null;for(AccessibilityNodeInfo n:matches){if(hit==null&&n.isVisibleToUser()&&!n.isPassword()&&semantic(n,copy))hit=n;else n.recycle();}if(hit!=null){root.recycle();return hit;}}
    ...
    private boolean click(AccessibilityNodeInfo node,int token)throws Exception{
        ...
        for(int i=0;n!=null&&i<7;i++){check(token);if(n.isClickable()&&n.isEnabled()&&n.performAction(AccessibilityNodeInfo.ACTION_CLICK)){n.recycle();return true;}AccessibilityNodeInfo p=n.getParent();n.recycle();n=p;}if(n!=null)n.recycle();
    ...
    private void capture(){
        if(busy)return;cancel();busy=true;final int token=generation;render();
        operation=reader.submit(()->{try{
            ...
            if(!social(owner)&&!browser(owner))throw new Exception("请在浏览器、小红书或抖音的作品页使用");
            String value;
            if(browser(owner))value=browserLink(owner,token);else{
                AccessibilityNodeInfo copy=button(owner,true,token);
                if(copy==null){AccessibilityNodeInfo share=button(owner,false,token);if(share==null||!click(share,token))throw new Exception("没有识别到分享控件，可打开分享菜单后再点采集");long until=SystemClock.uptimeMillis()+5000;while(copy==null&&SystemClock.uptimeMillis()<until){Thread.sleep(180);check(token);copy=button(owner,true,token);}}
        ...
        }catch(InterruptedException ignored){}catch(Exception e){handler.post(()->{if(token==generation)message(e.getMessage()==null?"采集未完成，请重试":e.getMessage());});}});
    }
    ...
    private String copiedLink(String owner,long after,int token)throws Exception{
        CompletableFuture<Boolean> focus=new CompletableFuture<>();handler.post(()->{if(token!=generation||bubble==null){focus.complete(false);return;}layout.flags=WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL;manager.updateViewLayout(bubble,layout);bubble.setFocusableInTouchMode(true);bubble.requestFocus();focus.complete(true);});if(!focus.get(2,TimeUnit.SECONDS))throw new InterruptedException();
        ClipboardManager clipboard=(ClipboardManager)getSystemService(CLIPBOARD_SERVICE);
        for(int i=0;i<12;i++){Thread.sleep(150);check(token);try{ClipDescription d=clipboard.getPrimaryClipDescription();ClipData c=d!=null&&d.getTimestamp()>=after?clipboard.getPrimaryClip():null;String value=c!=null&&c.getItemCount()>0?link(String.valueOf(c.getItemAt(0).getText())):"";String host=Uri.parse(value).getHost();if(host!=null&&(owner.equals("com.xingin.xhs")?host.matches("(^|.*\\.)(xiaohongshu\\.com|xhslink\\.com)"):host.matches("(^|.*\\.)(douyin\\.com|iesdouyin\\.com)")))return value;}catch(SecurityException ignored){}}
        throw new Exception("系统未提供本次复制的链接，请重试采集");
    }
    private void open(String value){cancel();expanded=false;render();try{startActivity(new Intent(this,ShareActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK).putExtra(Intent.EXTRA_TEXT,value));}catch(Exception e){message("无法打开采集页，请重试");}}
}
