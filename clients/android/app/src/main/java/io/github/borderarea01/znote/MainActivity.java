...
public class MainActivity extends Activity {
    ...
    private void showConnect(String error){
        ...
        panel.addView(text("这是连接同一知识库的客户端，未连接时不保存整库离线副本。不要将其他设备的 localhost 当作服务器地址。",13,Color.rgb(160,174,198)));panel.addView(button("检查应用更新",()->startActivity(new Intent(this,UpdateActivity.class))));panel.addView(button("安装帮助与反馈 ↗",()->external("https://github.com/BorderArea01/ZNote/blob/main/docs/CLIENTS.md")));
    }
    ...
    private void open(String target){
        ...
        Button back=button("‹",this::handleBack);back.setContentDescription("返回上一层");back.setMinWidth(dp(40));back.setMinimumWidth(dp(40));toolbar.addView(back,new LinearLayout.LayoutParams(dp(44),-1));Button reload=button("↻",()->{if(web!=null)web.evaluateJavascript("window.ZNoteNavigation?window.dispatchEvent(new Event('znote:refresh')):location.reload()",null);});reload.setContentDescription("刷新知识库内容");toolbar.addView(reload,new LinearLayout.LayoutParams(dp(44),-1));TextView title=text("ZNote",16,Color.rgb(35,45,62));toolbar.addView(title,new LinearLayout.LayoutParams(0,-2,1));toolbar.addView(button("采集",()->startActivity(new Intent(this,ShareActivity.class))));Button more=button("⋯",()->{});more.setContentDescription("更多选项");toolbar.addView(more,new LinearLayout.LayoutParams(dp(44),-1));more.setOnClickListener(v->{PopupMenu menu=new PopupMenu(this,more);menu.getMenu().add("检查应用更新").setOnMenuItemClickListener(item->{startActivity(new Intent(this,UpdateActivity.class));return true;});menu.getMenu().add("采集设置").setOnMenuItemClickListener(item->{startActivity(new Intent(this,CaptureAssistActivity.class));return true;});menu.getMenu().add("切换连接").setOnMenuItemClickListener(item->{new AlertDialog.Builder(this).setMessage("切换前请保存当前编辑。").setNegativeButton("取消",null).setPositiveButton("切换",(d,w)->showConnect("")).show();return true;});menu.show();});
