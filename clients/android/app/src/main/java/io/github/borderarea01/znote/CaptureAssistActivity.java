...
public class CaptureAssistActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state){
        ...
        root.addView(button("显示悬浮窗",()->{getSharedPreferences("MainActivity",0).edit().putBoolean("capture_bubble",true).apply();if(CaptureAssistService.current!=null)CaptureAssistService.current.showBubble();update();}));
        root.addView(button("关闭悬浮窗",()->{getSharedPreferences("MainActivity",0).edit().putBoolean("capture_bubble",false).apply();if(CaptureAssistService.current!=null)CaptureAssistService.current.hideBubble();update();}));
        root.addView(button("粘贴链接采集",()->startActivity(new Intent(this,ShareActivity.class))));
        root.addView(button("返回",this::finish));
    }
    private void update(){status.setText(CaptureAssistService.current==null?"采集辅助未开启":"采集辅助已连接 · "+(getSharedPreferences("MainActivity",0).getBoolean("capture_bubble",true)?"悬浮窗已显示":"悬浮窗已关闭"));}
