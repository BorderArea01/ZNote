...
async function get(request,url,hash){const r=await request(url,{signal:AbortSignal.timeout(180000)});if(!r.ok)throw Error('组件下载失败：HTTP '+r.status);const b=Buffer.from(await r.arrayBuffer());if(createHash('sha256').update(b).digest('hex')!==hash)throw Error('组件校验失败，未安装');return b}
async function install(data,request=fetch){if(await installed(data))return;const platform=process.platform+'-'+process.arch,digests=ff[platform];if(!digests)throw Error('此平台请配置系统视频组件');const p=paths(data);await mkdir(p.dir,{recursive:true});
 ...
 const license=await get(request,base+platform+'.LICENSE',digests[1]),readme=await get(request,base+platform+'.README',digests[2]);
 const compressed=await get(request,base+'ffmpeg-'+platform+'.gz',digests[0]);const binary=gunzipSync(compressed,{maxOutputLength:180*1024*1024});
 ...
 const win=process.platform==='win32',asset=win?'yt-dlp.exe':'yt-dlp_macos';const yt=await get(request,'https://github.com/yt-dlp/yt-dlp/releases/download/2026.08.19/'+asset,win?'66674953fe251b89f4d08c5f0e35e0728679bd67ab3d7d05c0562af101dd3e7a':'0f192b7ec147ab6288885d6351d9ab67367640029b4377576ef46dd79cf7b202');
