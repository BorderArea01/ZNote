...
export default function GroupOrganizeDialog({ items, kind = 'image', library, onClose, onDone }) {
  const mediaLabel = kind === 'video' ? '视频' : '图片';
  const mediaCountLabel = kind === 'video' ? '个视频' : '张图片';
  ...
  const input = { items: selection, collection_id: library, kind, mode, title, whole_groups: whole, target_id: target?.id || null, note_mode: notes };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => api('/api/item-groups?' + new URLSearchParams({ collection: library || 'unfiled', kind, q: query, offset }), { signal: controller.signal }).then(value => { if (!controller.signal.aborted) setGroups(value); }).catch(e => { if (!controller.signal.aborted) setGroupError(e.message); }), 150);
    return () => { clearTimeout(timer); controller.abort(); };
  }, [mode, kind, library, query, offset, revision]);
  useEffect(() => {
  ...
  return <Dialog title={`整理${mediaLabel}组`} className="group-organize-dialog" onClose={() => !busy && onClose()}>
    <div className="group-organize-body">
      <fieldset className="group-organize-settings" disabled={busy || uncertain}>
        <label>整理方式<select aria-label={`${mediaLabel}组整理方式`} value={mode} onChange={e => setMode(e.target.value)}><option value="create">组成新{mediaLabel}组</option><option value="append">追加到已有{mediaLabel}组</option><option value="detach">拆为独立{mediaLabel}</option></select></label>
        {mode === 'create' && <label>新{mediaLabel}组名称（可选）<input aria-label={`新${mediaLabel}组名称`} maxLength={200} placeholder={plan?.title ? `默认：${plan.title}` : `留空使用封面${mediaLabel}标题`} value={title} onChange={e => setTitle(e.target.value)}/></label>}
        {mode === 'append' && <div className="group-target-picker"><label>查找目标组<input aria-label={`搜索目标${mediaLabel}组`} value={query} onChange={e => { setQuery(e.target.value); setOffset(0); }} placeholder={`搜索当前知识库的${mediaLabel}素材组`}/></label>
          <select aria-label={`目标${mediaLabel}组`} value={target?.id || ''} onChange={e => setTarget(groups.groups.find(g => g.id === e.target.value) || null)}>
            <option value="">选择素材组</option>{target && !groups.groups.some(g => g.id === target.id) && <option value={target.id}>{target.title} · {target.count} {mediaCountLabel}</option>}{groups.groups.map(g => <option key={g.id} value={g.id}>{g.title} · {g.count} {mediaCountLabel}</option>)}
          </select>
        ...
        <div className="group-whole-row"><label className="group-whole"><input type="checkbox" checked={whole} onChange={e => setWhole(e.target.checked)}/>包含所选{mediaLabel}所在的整组</label><HelpHint label="整理范围">默认仅整理选中的内容。勾选后包含这些组中被筛选隐藏、或尚未加载的成员；每个原组保持内部顺序。追加时保留目标封面，新内容排在末尾。</HelpHint></div>
      ...
      <section className="group-plan" aria-label={`${mediaLabel}组整理预览`}>
        <div className="group-plan-heading"><strong>{mode === 'detach' ? `将拆为独立${mediaLabel}` : plan?.title || `整理后的${mediaLabel}组`}</strong><span>{plan ? `${plan.result_count} ${mediaCountLabel}` : `${selection.length} ${mediaCountLabel}已选`}</span></div>
        {loading && <p role="status">正在核对内容与分组…</p>}
        {!loading && !plan && !error && mode === 'append' && <p className="muted">选择目标组后显示预览</p>}
        {plan && <><p className="group-plan-summary">已选 {plan.selected_count} {mediaCountLabel} · 实际范围 {plan.expanded_count} {mediaCountLabel}{plan.target_count ? ` · 目标原有 ${plan.target_count} ${mediaCountLabel}` : ''}</p>
