...
export function GalleryStrip({items=[],index,onSelect,busy=false}) {
  ...
  return <nav className={`gallery-strip${expanded?' is-expanded':''}`} aria-label={`${label}缩略图列`} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} onFocus={event=>{if(!event.target.closest('.gallery-strip-toggle'))setHovered(true);}} onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget)&&!pinned)setHovered(false);}}>
