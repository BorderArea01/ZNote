...
export function SelectionBar({ count, loadedCount, allLoaded, someLoaded, total, locked, progress, working, trash, groupKind, imagesOnly, allFavorite, showStart, showPrevious, startLabel, previousLabel, onStart, onPrevious, onLoaded, onAll, onInvert, onClear, onExit, onCancel, onTags, onOrganize, onGroup, onFavorite, onTrash, onPurge }) {
  const disabled = !count || locked;
  const resolvedGroupKind = groupKind || (imagesOnly ? 'image' : null);
  const mediaLabel = resolvedGroupKind === 'video' ? '视频' : '图片';
  return <section className="selection-bar batch-toolbar" aria-label="批量管理">
    <div className="selection-summary">
      ...
      <HelpHint label="多选快捷操作">图片组和视频组保持折叠，点击组卡片选择或取消整组（含筛选隐藏、未加载的成员），「选择组内图片」或「选择组内视频」可单独挑选。Shift 点击连续选择，Ctrl / ⌘ 点击直接进入多选。当前页全选和反选按卡片操作；Ctrl / ⌘ A 只全选当前筛选匹配的内容。Esc 清除选择，再按一次退出。单次最多 10000 项。</HelpHint>
