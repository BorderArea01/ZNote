...
export function GroupOrderDialog({id,onClose,onDone}) {
  ...
  useEffect(()=>{let active=true;api('/api/item-groups/order?id='+encodeURIComponent(id)).then(value=>{if(active){setSnapshot(value);setItems(value.items)}}).catch(e=>active&&setError(e.message));return()=>{active=false}},[id]);
  ...
  return <Dialog title="调整图片顺序" className="group-order-dialog" onClose={close}>
    ...
    <div className="order-grid" aria-label="图片排序列表">
      {items.slice(page*pageSize,(page+1)*pageSize).map((item,local)=>{const index=page*pageSize+local;return <article key={item.id} data-order-id={item.id} className={`order-card ${drag===item.id?'dragging':''}`} draggable={!busy}
        ...
        <div className="order-card-actions">
          <button disabled={busy||index===0} aria-label={`将第 ${index+1} 张前移`} onClick={()=>move(item.id,items[index-1].id)}><ArrowLeft size={15}/></button>
          <button className="order-grip" aria-label={`拖动第 ${index+1} 张`} disabled={busy} title="拖动调整顺序"
          ...
          <button disabled={busy||index===items.length-1} aria-label={`将第 ${index+1} 张后移`} onClick={()=>move(item.id,items[index+1].id)}><ArrowRight size={15}/></button>
        </div>
    ...
    <div className="order-footer">
      ...
      <div className="order-actions"><button onClick={close} disabled={busy}>取消</button><button className="primary" disabled={busy||!dirty} onClick={save}>{busy?'正在保存…':'保存顺序'}</button></div>
    </div>
  </Dialog>;
}
