...
import './tasks.css';
const Context=createContext(null),live=s=>['ready','queued','running','saving'].includes(s);
...
export function TaskProvider({children}){
  ...
  useEffect(()=>{
    ...
    const poll=async()=>{
      ...
      const results=await Promise.allSettled([api('/api/imports',{signal:controller.signal}),api('/api/backups',{signal:controller.signal}),api('/api/export-jobs',{signal:controller.signal})]);
      if(!disposed)store.setRemote({imports:results[0].status==='fulfilled'?results[0].value.jobs:store.remote.imports,backup:results[1].status==='fulfilled'?results[1].value:store.remote.backup,exports:results[2].status==='fulfilled'?results[2].value.jobs:store.remote.exports,error:results.some(r=>r.status==='rejected')?'部分服务器状态未能更新，可刷新重试；下方保留上次结果':null});
      inflight=false;if(!disposed)timer=setTimeout(poll,store.remote.error?15000:store.watchers||store.remote.backup?.busy||[...store.remote.imports,...store.remote.exports].some(j=>live(j.status))?2500:30000);
    };
...
export async function startExport(store,query,retryId) {
  ...
  if(job.status==='ready') {
    const a=document.createElement('a');a.href=job.download_url;a.download='';a.hidden=true;
    document.body.append(a);a.click();a.remove();
  }
  store.refreshRemote();return job;
}
export function TaskButton({onClick}){const {jobs,remote}=useTaskSnapshot();const count=jobs.filter(j=>live(j.status)).length+[...remote.imports,...remote.exports].filter(j=>live(j.status)).length+(remote.backup?.busy&&!jobs.some(j=>j.type==='backup'&&live(j.status))?1:0),failed=jobs.some(j=>j.status==='failed')||[...remote.imports,...remote.exports].some(j=>j.status==='failed'&&!j.retried_as)||remote.error;return <button className="task-button" aria-label="任务中心" onClick={onClick}><ListTodo size={18}/><span className="task-button-label">任务</span>{count>0?<b>{count}</b>:failed?<span className="task-dot"/>:null}</button>}
