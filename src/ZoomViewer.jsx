...
export function ZoomViewer({src,alt,onClose}) {
  ...
  const outsideImage=e=>{const rect=area.current.querySelector('img').getBoundingClientRect();return e.clientX<rect.left||e.clientX>rect.right||e.clientY<rect.top||e.clientY>rect.bottom;};
  const startDrag=e=>{if(e.button!==0)return;drag.current={px:e.clientX,py:e.clientY,x:view.x,y:view.y,outside:outsideImage(e)};e.currentTarget.setPointerCapture(e.pointerId);};
  const endDrag=e=>{const start=drag.current;drag.current=null;if(start?.outside&&Math.hypot(e.clientX-start.px,e.clientY-start.py)<5&&outsideImage(e))onClose();};
  ...
  return <div className="lightbox zoom-viewer" role="dialog" aria-label="展开图片" aria-modal="true" onKeyDown={e=>{if(e.key==='Escape'){e.stopPropagation();onClose()}}}>
    <div ref={area} className="zoom-surface" tabIndex={0} onPointerDown={startDrag} onPointerMove={e=>{if(drag.current)setView(v=>({...v,x:drag.current.x+e.clientX-drag.current.px,y:drag.current.y+e.clientY-drag.current.py}))}} onPointerUp={endDrag} onPointerCancel={()=>drag.current=null} onDoubleClick={()=>setView({scale:1,x:0,y:0})}>
