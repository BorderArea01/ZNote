...
export function GroupSelectionDialog({group,selected,onClose,onApply,onDetach,onDetached}){
  ...
  const pending=useRef(null),noteGroup=(group.groupKey||group.rows[0]?.group_key||'').startsWith('note:'),videoGroup=group.rows[0]?.kind==='video',mediaLabel=videoGroup?'视频':'图片';
  ...
  return <Dialog title={`选择组内${mediaLabel}`} className="group-selection-dialog" onClose={()=>!busyId&&onClose()}>
    <div className="group-selection-body"><div className="inline-heading"><strong>{group.title}</strong><HelpHint label="组内操作">点击内容可调整本次批量选择；“移出组”只拆出对应内容并可撤销，不改变其他内容的顺序和封面。{noteGroup?'笔记配图请在笔记正文中增删，避免正文引用与配图组不一致。':videoGroup?'视频组支持整组选择、收藏、移动和删除；单个视频仍可单独操作。':''}</HelpHint></div>
      <div className="group-selection-summary"><span>已选 {ids.size} / {group.rows.length} 个{mediaLabel}</span><button onClick={()=>setIds(new Set(group.rows.map(row=>row.id)))}>全选组内{mediaLabel}</button><button onClick={()=>setIds(new Set())}>清空组内选择</button></div>
      <div className="group-selection-grid">{rows.map((row,index)=>{const position=page*60+index+1;return <div key={row.id} className={`group-selection-card${ids.has(row.id)?' is-selected':''}`}><label><img src={`/media/${row.id}/thumbnail`} alt={row.title} loading="lazy"/><span><input type="checkbox" aria-label={`选择第 ${position} 个${mediaLabel}`} checked={ids.has(row.id)} disabled={!!busyId} onChange={()=>setIds(previous=>{const next=new Set(previous);next.has(row.id)?next.delete(row.id):next.add(row.id);return next;})}/>{position}{position===1?` · 首${mediaLabel}`:''}</span></label>{!noteGroup&&!videoGroup&&<button className="group-detach-button" disabled={!!busyId} onClick={()=>detach(row,position-1)} aria-label={`${busyId===row.id&&pending.current?.id===row.id?'重试移出':'移出'}第 ${position} 张图片`}><Unlink size={14}/>{busyId===row.id?'正在移出…':pending.current?.id===row.id?'重试移出':'移出组'}</button>}</div>})}</div>
