...
function read() { try { const text=localStorage.getItem(KEY)||'{}';if(text.length>64000)return {};const value=JSON.parse(text);return value&&typeof value==='object'&&!Array.isArray(value)?value:{}; } catch { return {}; } }
...
export class ReadingSync {
  constructor(library,onChange,request=api) {
    this.library=library;this.key=library||'unfiled';this.owner=uuid();this.onChange=onChange;this.request=request;
    const saved=read()[this.key];this.active=saved?.active||null;this.next=saved?.next||null;
  ...
  persist(claim=false) {
    try {
      const map=read();if(!claim&&map[this.key]?.owner&&map[this.key].owner!==this.owner)return;
  ...
  async flush() {
    ...
    try {
      const data=await this.request('/api/reading-progress',{method:action.method,body:JSON.stringify(action.body),keepalive:true,signal:this.controller.signal});
  ...
  async replaceWithLatest() {
    ...
    this.active={method:clearing?'DELETE':'POST',body:{collection_id:this.library,version:this.state.data.version,epoch:this.state.data.epoch,request_id:uuid(),...(!clearing?{item_id:candidate.item_id}:{})}};this.next=null;this.persist(true);await this.flush();
  }
  async clear() {
