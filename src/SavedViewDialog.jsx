...
function Conditions({ config }) {
  return <div className="saved-view-conditions" aria-label="保存的筛选条件">
    <div className="saved-view-chips"><span>{savedViewLabels[config.view]}</span><span>{{ updated:'最近更新', created:'最近创建', title:'名称排序' }[config.sort]}</span><span>{config.layout === 'grid' ? '网格' : '列表'}</span></div>
