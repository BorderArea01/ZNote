...
test('saved views are scoped, validated, conflict-aware, durable and included in complete restoration', async t => {
  ...
  runtime.db.exec('DROP TABLE reading_progress; DROP TABLE group_operations; DROP TABLE saved_views; PRAGMA user_version=9');
...
test('schema 9 upgrades additively and repeated startup preserves existing rows', async () => {
  ...
  db.exec('DROP TABLE reading_progress; DROP TABLE group_operations; DROP TABLE saved_views; PRAGMA user_version=9');
