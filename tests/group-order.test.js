...
test('image order persists independently of Pixiv page identity, rejects stale/cross-library sets, synchronizes notes and restores',async t=>{
 ...
 await backup(runtime.db,join(dir,'schema6.sqlite'));await copyFile(join(dir,'schema6.sqlite'),join(dir,'znote.sqlite'));let db=new DatabaseSync(join(dir,'znote.sqlite'));db.exec('DROP TABLE pending_file_deletions; ALTER TABLE items DROP COLUMN group_order; PRAGMA user_version=5');const old=db.prepare('SELECT * FROM items ORDER BY id').all();db.close();
 db=openDatabase(dir);assert.equal(db.prepare('PRAGMA user_version').get().user_version,7);assert.deepEqual(db.prepare('SELECT * FROM items ORDER BY id').all().map(({group_order,...r})=>{assert.equal(group_order,null);return r}),old.map(r=>({...r})));db.close();db=openDatabase(dir);assert.equal(db.prepare('PRAGMA user_version').get().user_version,7);db.close();
});
