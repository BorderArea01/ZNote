...
test('regrouping 10000 pages has bounded preview output and upgrades schema 10 without rewriting data', async t => {
  ...
  db.exec('DROP TABLE group_operations; DROP INDEX items_group_origin; ALTER TABLE items DROP COLUMN group_manual; ALTER TABLE items DROP COLUMN group_origin_id; PRAGMA user_version=10');
  ...
  assert.equal(db.prepare('PRAGMA user_version').get().user_version, 11); assert.deepEqual(db.prepare('SELECT * FROM items').all().map(({group_manual,group_origin_id,...row}) => { assert.equal(group_manual,0); assert.equal(group_origin_id,null); return row; }), before); db.close();
