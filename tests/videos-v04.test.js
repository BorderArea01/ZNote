...
test('schema 3 migration preserves rows, original references and FTS while admitting videos', async () => {
  ...
  try { const { duration, video_codec, group_key, group_index, group_title, group_order, ...after } = next.prepare('SELECT rowid,* FROM items').get(); assert.deepEqual(after, { ...before }); assert.equal(next.prepare('PRAGMA user_version').get().user_version, 7); assert.equal(next.prepare("SELECT count(*) n FROM items_search WHERE items_search MATCH '迁移前'").get().n, 1); next.prepare("INSERT INTO items(id,kind,title) VALUES(?,'video','视频')").run(randomUUID()); }
  finally { next.close(); }
});
