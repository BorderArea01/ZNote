...
try{
 ...
 await page.locator('.sort-trigger').click();await page.getByLabel('排序方式').selectOption('title');await page.getByLabel('排序方向').selectOption('desc');await page.getByRole('checkbox',{name:'按类型分区',exact:true}).check();await page.waitForTimeout(300);await page.waitForFunction(id=>document.querySelector('.item-card')?.dataset.itemId===id,singleB.id);
 await page.getByLabel('排序方向').selectOption('asc');await page.getByRole('button',{name:'完成',exact:true}).click();await page.waitForTimeout(300);await page.waitForFunction(id=>document.querySelector('.item-card')?.dataset.itemId===id,singleA.id);
