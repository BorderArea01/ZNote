...
try {
 ...
 assert.deepEqual(result.group.images,[0,1,2].map(n=>`https://i.pximg.net/img-original/12345_p${n}.png`));assert.equal(result.calls,2);assert.equal(result.group.previews[1],'https://i.pximg.net/img-master/12345_p1.jpg');assert.equal(result.group.start_index,1);
 assert.equal(result.other.source_url,'https://www.pixiv.net/artworks/98765');assert.ok(result.other.images.every(url=>url.includes('/98765_')),'Hovered recommendation resolves its own work, never merges into current work');
