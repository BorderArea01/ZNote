...
try {
  ...
  const help = page.getByRole('button',{name:'标签筛选说明',exact:true});
  assert.equal(await page.getByRole('tooltip').count(),0);
  await help.hover(); await page.getByRole('tooltip').waitFor();
  assert.ok(await page.getByRole('tooltip').textContent().then(s=>s.includes('全部或任意')));
  await page.getByRole('tooltip').hover(); await page.waitForTimeout(250);
  assert.equal(await page.getByRole('tooltip').count(),1, 'Help remains readable when hovered');
  ...
  await page.keyboard.press('Escape'); assert.equal(await page.getByRole('tooltip').count(),0);
  await page.screenshot({path:resolve('artifacts/v082-clean-library.png')});
  const touchContext = await browser.newContext({viewport:{width:390,height:844},isMobile:true,hasTouch:true,storageState:await context.storageState()});
  const touchPage = await touchContext.newPage(); await touchPage.goto(base);
  await touchPage.getByRole('button',{name:'标签筛选说明',exact:true}).tap();
  await touchPage.getByRole('tooltip').waitFor();
  const bubbleBox=await touchPage.getByRole('tooltip').boundingBox();
  assert.ok(bubbleBox.x>=0 && bubbleBox.x+bubbleBox.width<=390);
  await touchPage.screenshot({path:resolve('artifacts/v082-mobile-help.png')});
  await touchPage.getByRole('button',{name:'标签筛选说明',exact:true}).tap();
  assert.equal(await touchPage.getByRole('tooltip').count(),0);
  await touchContext.close();
  console.log('PASS: contextual help supports pointer, tooltip hover, keyboard focus/Escape and mobile tap without clipping');
  await page.getByRole('button', { name: '设置与连接', exact: true }).click();
  await page.getByRole('button',{name:'外观设置说明',exact:true}).focus();
  await page.getByRole('tooltip').waitFor(); await page.keyboard.press('Escape');
  assert.equal(await page.getByRole('dialog',{name:'设置与连接'}).isVisible(),true);
  await page.screenshot({path:resolve('artifacts/v082-settings.png')});
  const colors = {};
