...
try {
  ...
  await page.setViewportSize({width:1440,height:1050});
  await page.locator('.sidebar').getByRole('button',{name:/^设计灵感 \d+$/}).click();
  await page.locator('.sidebar').getByRole('button',{name:/^全部内容 \d+$/}).click();
  let failedOnce=false;
  await page.route('**/api/assets',async route=>{if(!failedOnce&&route.request().method()==='POST'&&route.request().postData()?.includes('batch-second.png')){failedOnce=true;await route.abort('failed');}else await route.continue();});
  await page.locator('input[type=file]').first().setInputFiles([{name:'batch-cover.png',mimeType:'image/png',buffer:png},{name:'batch-second.png',mimeType:'image/png',buffer:png}]);
  assert.ok(await page.getByLabel('上传后组成图片组',{exact:true}).isChecked());
  await page.screenshot({path:'artifacts/upload-group-choice.png'});
  await page.getByRole('button',{name:'开始上传',exact:true}).click();
  await page.getByRole('button',{name:'重试失败 / 继续上传',exact:true}).waitFor();
  assert.ok(failedOnce);
  await page.getByRole('button',{name:'重试失败 / 继续上传',exact:true}).click();
  await page.getByRole('button',{name:'完成',exact:true}).click();
  await page.unroute('**/api/assets');
  await page.getByRole('button',{name:'打开 batch-cover.png',exact:true}).waitFor();
  const groupedUpload=await (await context.request.get(base+`/api/items?collection=${library.id}&grouped=true`)).json();
  const batchGroup=groupedUpload.items.find(i=>i.group_title==='batch-cover.png');
  assert.equal(batchGroup.group_count,2);
  const pages=await (await context.request.get(base+`/api/items?collection=${library.id}&group_key=${encodeURIComponent(batchGroup.group_key)}`)).json();
  assert.deepEqual(pages.items.map(i=>i.group_index).sort(),[0,1]);
  await page.screenshot({path:'artifacts/upload-group-result.png'});
  checkpoint('grouped batch upload, same-file reuse and partial failure retry retain one ordered group');
  const uploadTouch=await browser.newContext({storageState:await context.storageState(),viewport:{width:390,height:844},isMobile:true,hasTouch:true});
  await uploadTouch.addInitScript(()=>Object.defineProperty(crypto,'randomUUID',{value:undefined}));
  const uploadPage=await uploadTouch.newPage();
  await uploadPage.goto(base);
  await uploadPage.getByRole('button',{name:'打开导航'}).tap();
  await uploadPage.locator('.sidebar').getByRole('button',{name:/^设计灵感 \d+$/}).tap();
  await uploadPage.locator('input[type=file]').first().setInputFiles([{name:'touch-cover.png',mimeType:'image/png',buffer:png},{name:'touch-second.png',mimeType:'image/png',buffer:png}]);
  const groupChoice=uploadPage.getByLabel('上传后组成图片组',{exact:true});
  await groupChoice.tap();assert.equal(await groupChoice.isChecked(),false);
  await groupChoice.tap();assert.equal(await groupChoice.isChecked(),true);
  await uploadPage.screenshot({path:'artifacts/upload-group-touch.png'});
  await uploadPage.getByRole('button',{name:'开始上传',exact:true}).tap();
  await uploadPage.getByRole('button',{name:'完成',exact:true}).tap();
  await uploadPage.getByRole('button',{name:'打开 touch-cover.png',exact:true}).waitFor();
  const retained=await (await context.request.get(base+`/api/items?collection=${library.id}&group_key=${encodeURIComponent(batchGroup.group_key)}`)).json();
  assert.equal(retained.items.length,2);
  await uploadTouch.close();
  checkpoint('touch grouping choice and upload preserve pre-existing groups');
  console.log(
