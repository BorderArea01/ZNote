...
try {
  ...
  await page.getByLabel('恢复快照间隔小时').fill('6'); await page.getByLabel('恢复快照保留版本数').fill('3');
  await page.getByRole('button', { name: '保存快照设置', exact: true }).click();
  await page.getByRole('button', { name: '立即创建快照', exact: true }).click();
  await page.getByRole('link', { name: '下载迁移 ZIP', exact: true }).waitFor();
  ...
  const downloadPromise = page.waitForEvent('download'); await page.getByRole('link', { name: '下载迁移 ZIP', exact: true }).click();
  ...
  await page.getByLabel('导入迁移备份 ZIP').setInputFiles(downloaded);
