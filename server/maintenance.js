...
export function maintenanceGate(app) {
  ...
  app.use((req, res, next) => {
    if (locked && req.path !== '/api/health') return res.status(503).json({ error: '正在恢复备份，请稍后重新登录' });
    responses.add(res);
    ...
    app[method] = (path, ...route) => {
      ...
      return register(path, ...route, async (req, res, next) => {
        if (locked && req.path !== '/api/health') {
          ...
          req.releaseStream?.();
          return res.status(503).json({ error: '正在恢复备份，请稍后重试' });
        }
  ...
  return {
    ...
    async work(operation) {
      if (locked) throw Object.assign(new Error('正在恢复备份，请稍后重新采集'), { status: 409 });
      const work = Promise.resolve().then(operation); background.add(work);
      try { return await work; } finally { background.delete(work); }
    },
    async exclusive(res, operation) {
      if (locked) throw Object.assign(new Error('另一项恢复正在进行'), { status: 409 });
      locked = true;
      try {
        const deadline = Date.now() + 30000;
        while (background.size || [...responses].some(r => r !== res) || [...handlers.values()].some(r => r !== res)) {
          if (Date.now() > deadline) throw Object.assign(new Error('仍有上传或下载正在进行，请完成后重试恢复'), { status: 409 });
