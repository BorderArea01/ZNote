...
const groupOrderState={type:'object',properties:{revision:str,kind:{type:'string',enum:['image','video']},collection_id:{...str,nullable:true},group_key:str,note_id:{...str,nullable:true},cover_id:str,items:list({type:'object',properties:{id:str,kind:{type:'string',enum:['image','video']},title:str,thumbnail_url:str,version:{type:'integer'}}}),item:ref('Item')}};
spec.paths['/api/item-groups/order']={
  get:operation('读取当前知识库媒体组排序快照',groupOrderState,{parameters:[{name:'id',in:'query',schema:str,description:'组内媒体或所属笔记 ID；可与稳定的 group_key、collection、kind 一起用于恢复旧卡片'},{name:'group_key',in:'query',schema:str},{name:'collection',in:'query',schema:str,description:'知识库 ID；unfiled 为未分类'},{name:'kind',in:'query',schema:{type:'string',enum:['image','video']}}]}),
  post:operation('原子保存媒体组顺序（write）',groupOrderState,{description:'提交全部有效成员，第一项为封面。revision 或组成员变化返回 409，整次回滚。sync_note 默认同步笔记正文图片顺序，保留周围文字和链接。',requestBody:body({type:'object',required:['id','revision','ids'],properties:{id:str,kind:{type:'string',enum:['image','video']},group_key:str,collection_id:{...str,nullable:true},revision:{type:'string',minLength:64,maxLength:64},ids:{type:'array',minItems:1,maxItems:10000,uniqueItems:true,items:str},sync_note:{type:'boolean',default:true}}})})
};
...
const organizeInput = {type:'object',required:['items','collection_id','mode'],properties:{
  ...
  collection_id:{type:'string',format:'uuid',nullable:true},kind:{type:'string',enum:['image','video'],description:'所选媒体类型；省略时从第一项推断，所有项必须同类型。'},mode:{type:'string',enum:['create','append','detach']},title:{type:'string',maxLength:200,default:'',description:'新组名称可省略；留空采用整理后实际封面标题，最多 200 字符。追加保留目标组名称。'},whole_groups:{type:'boolean',default:false},target_id:{type:'string',format:'uuid',nullable:true},note_mode:{type:'string',enum:['copy','exclude'],default:'copy'}
}};
...
spec.paths['/api/item-groups']={get:operation('分页查找当前知识库素材组',{type:'object',properties:{groups:list({type:'object',properties:{...organizePicture.properties,group_key:str,count:{type:'integer'},kind:{type:'string',enum:['image','video']}}}),total:{type:'integer'},offset:{type:'integer'}}},{parameters:[{name:'collection',in:'query',schema:{type:'string',default:'unfiled'},description:'知识库 UUID 或 unfiled'},{name:'kind',in:'query',schema:{type:'string',enum:['image','video'],default:'image'},description:'按图片组或视频组筛选'},{name:'q',in:'query',schema:{type:'string',maxLength:200}},{name:'offset',in:'query',schema:{type:'integer',minimum:0,maximum:100000,default:0}}],description:'每页 40 组，不含回收站与笔记配图组；首张按展示顺序确定。'})};
spec.paths['/api/item-groups/organize/preview']={post:operation('预览合组、追加或拆分图片组或视频组（write）',organizePreview,{requestBody:body({...organizeInput,properties:{...organizeInput.properties,refresh:{type:'boolean',default:false,description:'明确重新读取所选版本；默认拒绝旧版本'}}}),description:'同知识库活动媒体；create 可省略组名，留空采用实际封面标题；append 需同类型素材组目标。关联及结果组最多 10000 项，预览至多 60 项。笔记配图处理仅适用于图片。追加保留目标封面。'})};
spec.paths['/api/item-groups/organize']={post:operation('按预览原子整理图片组或视频组（write）',{type:'object',properties:{changed_count:{type:'integer'},copied_count:{type:'integer'},group_key:{...str,nullable:true},ids:list(str),item:{...ref('Item'),nullable:true},undo:{...undoAction,nullable:true},replayed:{type:'boolean'},already_undone:{type:'boolean'}}},{requestBody:body({...organizeInput,required:[...organizeInput.required,'revision','operation_id','prepared_at'],properties:{...organizeInput.properties,...organizeReceipt,undo:{type:'boolean',default:false}}}),description:'原样提交预览 input（展开）、revision、operation_id、prepared_at。网络失败使用完全相同请求重试；24 小时内同调用者不重复执行，已撤销也不重做。版本或组成员变化 409，过期 410，容量超限 413，均整次回滚。'})};
...
spec.paths['/api/item-groups/selection'].get.responses[200].content['application/json'].schema.properties.items.items.properties.kind={type:'string',enum:['image','video']};
...
weixinConfig.properties.capture_links={type:"boolean",description:"是否采集新微信消息中的网页链接，默认 false；旧队列保留接收时配置。"};
const captureRequest={type:"object",required:["text"],properties:{text:{type:"string",minLength:1,maxLength:16000},collection_id:{type:"string",nullable:true},image_mode:{type:"string",enum:["group","note"],default:"note",description:"平台图集保存方式：group 只保存图片，正文存备注；note 保存笔记和成组配图。普通网页文章和视频不受影响。"},tags:{type:"array",maxItems:20,items:{type:"string",maxLength:40}},request_id:{type:"string",pattern:"^[a-zA-Z0-9:_-]{1,160}$"}}};
