...
spec.paths["/api/captures"]={get:operation("读取持久化的手机/网页采集记录",{type:"object"}),post:operation("提交分享链接，由服务器归档图文或视频",{type:"object"},{requestBody:body(captureRequest),description:"需要 write 权限。每次一个链接；最多 16 个待处理任务、100 条记录。相同 request_id 去重。202 仅代表接收，须查询直到 completed 才是入库成功。图片上限 25 MB，视频上限 500 MB，正文配图最多 100 张；平台验证或部分配图失败时记录失败，重试复用已保存配图。重启中断任务保留为 failed。",responses:{202:response({type:"object"}),...errorResponses}})};
